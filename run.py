...
app = FastAPI(
    ...
    if not TESTING and not NO_RATE_LIMIT
    else None,
)
...
async def on_startup():
    if not NO_RATE_LIMIT:
        # REDIS_HOST is needed when launching in a container
...
if __name__ == "__main__":
    run("run:app", host="localhost", port=8008, reload=True)
